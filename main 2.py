...
from lesson_plan import lesson_bot
...
def main():
	try:
		...
		sac.divider(label='ETD & ITD MOE Project', icon='house', align='center', direction='horizontal')
		...
		with st.sidebar: #options for sidebar
			...
			if st.session_state.login == False:
				...
				image = Image.open('Metacog Logo.png')
				st.image(image)
				...
				st.session_state.option = sac.menu([
					...
					sac.MenuItem('Learning Tools', icon='tools', children=[
						...
						sac.MenuItem('Conversation Assistant', icon='people-fill', disabled=is_function_disabled('Conversation Assistant')),
						sac.MenuItem('Metacognitive Feedback', icon='bug-fill', disabled=is_function_disabled('Metacognitive Feedback')),
						sac.MenuItem('Reflective Peer', icon='people', disabled=is_function_disabled('Reflective Peer')),
						sac.MenuItem('Thinking Facilitator (Chatbot)', icon='chat-dots', disabled=is_function_disabled('Thinking Facilitator')),
					]),
		...
		#Dialogic Agent
			...
			#Metacog settings
			choice = sac.buttons([
								sac.ButtonsItem(label='Conversation', icon='person-hearts',color='green'),
								sac.ButtonsItem(label='Default', icon='person-fill',color='blue'),
								sac.ButtonsItem(label='Paragraph', icon='person-plus-fill',color='red'),
			...
			elif choice == "Default": #remove the chatbot template
				st.session_state.chatbot = st.session_state.metacog_default
			elif choice == "Paragraph":
				st.session_state.chatbot = st.session_state.paragraph
			#Metacog settings
			...
			if st.session_state.vs:#chatbot with knowledge base
				if raw_search == True:
					...
					if st.session_state.memoryless: #memoryless chatbot with knowledge base but no memory
						...
						basebot_qa_memory(META_BOT) #chatbot with knowledge base and memory
				...
				if st.session_state.memoryless: #memoryless chatbot with no knowledge base and no memory
					...
					basebot_memory(META_BOT) #chatbot with no knowledge base but with memory
			...
			if transcript:
				if assessment_type == "Transcribing No Assessment":
					...
					if subject and topic :
						assessment_prompt(transcript, assessment_type, subject, topic)
					else:
